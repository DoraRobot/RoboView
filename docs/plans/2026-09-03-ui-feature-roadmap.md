...
| **008** | **object-transform** — move/rotate/scale gizmos and drag transforms | **Registered (2026-09-03)** | **005 selection semantics** (selection = one subject across tree/viewport/properties; transform commands target the selected object); **004 per-object appearance uniform channel** (gizmo handles and affected-object state build on the same per-object rendering state) |
| **009** | **timeline** — scrub / playback panel | **Registered (2026-09-03)** | **006 dockable/reserved-panel mechanism** (panel registry, multi-surface floating, layout persistence) to host the timeline panel |

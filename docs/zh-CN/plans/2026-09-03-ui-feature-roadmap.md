...
| **008** | **object-transform** —— 移动/旋转/缩放 Gizmo 与拖拽变换 | **Draft（spec 已创建 2026-09-03）** | **005 选中语义**（选中=树/视口/属性同一主语；变换命令作用于选中对象）；**004 per-object 外观 uniform 通道**（Gizmo 手柄与受影响对象状态同源渲染） |
| **009** | **timeline** —— scrub/回放面板 | **Draft（spec 已创建 2026-09-03）** | **006 可停靠/预留面板机制**（面板注册表、多 surface 浮动、布局持久化）以承载时间线面板 |
...
- 008/009 于 2026-09-03 由负责人确认：基于对成熟 3D 工具约定的差距分析
  ...
  同日创建 spec 草稿：`docs/specs/008-object-transform/` 与 `docs/specs/009-timeline/`
  （Draft，决策点待议）。
